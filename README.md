...
| (2026.08.06) Postgres 백엔드로 돌리다 파이프라인 도중 죽으면(Ctrl-C 등) 죽은 시점의 리뷰가 이전 분석 결과까지 영구 소실됨 | `store/db.py`의 `_PgConn`이 `autocommit=True`로 연결돼 있어 문장마다 즉시 개별 커밋됐음 — "delete+insert가 한 커밋 안에서 원자적으로 끝난다"는 SQLite 쪽 전제가 Postgres에선 애초에 성립하지 않았음(`commit()`이 사실상 no-op). `autocommit=False`로 바꾸고 `commit()`이 실제로 커밋하도록 수정, 문장 실패 시 트랜잭션이 aborted 상태로 멈추지 않도록 즉시 rollback도 추가(`tests/test_pgconn_transaction.py`로 검증) |
| (2026.08.06) `python pipeline.py` 실행 중 리뷰 하나의 LLM 분석이 걸리는 동안에도 여전히 DB 쓰기 락을 쥐고 있어 동시 `/api/feedback` 요청이 지연됨(리뷰당 커밋으로 바꾼 이전 수정으로도 완전히 해결 안 됨) | `delete`가 `analyze()` 호출 *전에* 실행돼 그 순간부터 이미 쓰기 트랜잭션(락)이 열려 있었음. `analyze()`를 트랜잭션 밖으로 빼 결과를 먼저 확보한 뒤에만 delete+insert+commit을 짧게 실행하도록 순서 변경. 같은 문제가 `chatbot.py`의 상품요약·광고카피·리뷰답글 사전계산 함수(상품 전체 루프가 끝나야 커밋 1번)에도 있어 함께 아이템별 커밋으로 수정. 동시쓰기 스레드를 띄우는 테스트로, 수정 전엔 분석이 끝날 때까지(약 0.93초) 블로킹되던 것이 수정 후 즉시 끝나는 것을 확인(`tests/test_pipeline_lock.py`) |
| (2026.08.06) LLM 응답 생성이 실패하면(Ollama 다운 등) 사용자에게 `(LLM 응답 생성을 건너뜀: ConnectionRefusedError...)`처럼 내부 예외가 그대로 노출됨 | `ground()` 예외는 이전에 사용자 노출 없이 로그로만 남기도록 고쳤는데, 그 뒤의 `_generate()`/`_stream_tokens()` 호출 실패는 같은 조치가 안 돼 있었음. 동일하게 서버 로그에만 원인을 남기고 사용자에겐 일반 안내 메시지로 통일(`ask`/`ask_stream` 둘 다) |
| (2026.08.06) 스트리밍 답변을 다 받은 직후 정정 메모리 조회(`recall_corrections`)나 캐시 저장(`_cache_put`)이 실패하면 `done` 신호 없이 스트림이 그냥 끊김 | 두 호출 모두 try/except 밖에 있었음. 각각 감싸서 실패해도(정정 조회는 빈 목록으로 대체, 캐시 저장은 생략) 이미 만든 답변과 함께 `done:true`는 항상 전송되도록 수정 |
| (2026.08.06) 비스트리밍(`/api/chat`)으로 추천·집계 같은 질문에 답한 뒤, 같은 질문을 스트리밍(`/api/chat/stream`)으로 다시 물으면 캐시에서 의도(intent)가 "리뷰 검색(RAG)"으로 잘못 표시됨 | `ask()`가 `_cache_put` 호출 두 곳(즉답 캐싱·LLM 생성 답변 캐싱) 모두 `mode` 인자를 빠뜨려 기본값 `"review"`로 저장하고 있었음(`ask_stream()`은 이미 올바르게 전달 중이었음). 두 호출 모두 `mode`를 넘기도록 수정, ground()/recall_corrections를 목으로 대체해 실제로 잘못된 intent가 저장되는 것과 수정 후 정상 저장되는 것을 재현·확인(`tests/test_chat_cache.py`) |
| (2026.08.06) `/api/feedback`(인증 없음)에 매우 긴 문자열을 반복 전송하면 DB가 무한정 커지고, 이후 모든 챗봇 응답이 점점 느려짐 | `Q`(질문) 모델엔 `max_length=500`이 있는데 `FB`(피드백) 모델엔 길이 제한이 전혀 없었음. `correction`은 매 챗봇 질문마다 `recall_corrections()`가 정정 문구가 있는 전체 행을 다시 임베딩하는 구조라, 거대한 값 하나가 그 이후 모든 응답을 느리게 만들 수 있었음. `q`(500자)·`answer`(4000자)·`correction`(1000자)·`vote`(10자)에 상한 추가 |
| (2026.08.06) 존재하지 않는 API 경로나 POST 전용 API(`/api/chat` 등)에 GET으로 접근하면 404/405 대신 200 + SPA HTML이 응답돼, 오류 모니터링이 이를 정상 요청으로 오인함 | SPA 캐치올(`GET /{full_path:path}`)이 모든 GET 요청에 매칭돼, API 라우터의 405/404보다 먼저 매치돼버림(주석엔 원래 "/api 외 모든 GET 경로"라고 의도가 적혀있었는데 실제 구현엔 그 제외 처리가 빠져있었음). `full_path`가 `api/`로 시작하면 SPA로 폴백하지 않고 404를 반환하도록 수정. `TestClient`로 `GET /api/chat`·`GET /api/존재안함`이 404로, 실제 SPA 라우트(`/dashboard`)와 실제 GET API(`/api/feedback/stats`)는 그대로 정상 응답하는 것을 확인(`tests/test_spa_fallback.py`) |
| (2026.08.06) `/cs`에서 챗봇 답변을 기다리는 도중 다른 페이지로 이동했다가 새 대화를 하고 돌아오면, 늦게 끝난 이전 스트림이 방금 저장된 새 대화 기록을 조용히 덮어써 사라짐 | `chatStream`의 `AbortController`가 함수 내부에만 있어 외부에서 취소할 방법이 없었고, `Chat.vue`엔 언마운트 훅도 없어 페이지를 벗어나도 스트림이 백그라운드에서 계속 진행됐음. `chatStream`에 외부 `signal` 옵션을 추가하고 `Chat.vue`가 언마운트 시 진행 중인 스트림을 즉시 `abort()`하도록 수정. 혹시 그 사이 스트림이 이미 끝나 `save()`가 호출되더라도 언마운트 이후엔 `localStorage`에 쓰지 않도록 플래그로 이중 방어 |
| (2026.08.06) 챗봇 답변이 토큰 단위로 실시간 타이핑되지 않고, 스트림이 다 끝난 뒤(정확히는 `busy` 상태가 바뀌는 순간) 전체 텍스트가 한 번에 나타남 | 스트리밍 콜백에서 다루는 `bot` 객체를 일반 객체로 `messages` 배열에 push해두고, 콜백에선 그 raw 참조를 직접 `bot.text += t`로 mutate하고 있었음 — Vue 3 반응성은 Proxy 기반이라 raw 참조를 직접 건드리면 set trap을 안 거쳐 화면 갱신이 트리거되지 않음(최종 값은 맞지만 그 순간엔 안 보이다가, 무관한 `busy` 값이 바뀌며 전체가 리렌더될 때 한 번에 나타난 것). `bot`을 `reactive()`로 감싸도록 수정. `@vue/reactivity`로 직접 재현 스크립트를 돌려, 수정 전엔 mutate 2번에도 렌더가 0회 추가되던 것이 수정 후엔 정확히 2회 추가되고 값도 즉시 반영되는 것을 확인 |
| (2026.08.06) `python eval.py llm` 실행 중 일부 리뷰 분석이 실패하면 원인 표시 없이 조용히 건너뛰어, 실제론 인프라 실패(Ollama 다운 등)인데 모델 품질이 낮은 것처럼 보임 | `except Exception: pass`로 완전히 삼키고 있었음(같은 실패를 다루는 `pipeline.py`는 원인을 출력함). 동일하게 실패 원인을 출력하도록 수정 |
...
| (2026.08.06) "리뷰 응대 가이드"의 복사 버튼을 클립보드 권한이 없는 환경에서 눌러도 복사 성공한 것처럼 체크 아이콘이 뜸 | `navigator.clipboard.writeText()`가 반환하는 Promise를 `await` 없이 바로 다음 줄(`copied.value = i`)을 실행하고 있었음(같은 기능을 하는 `Dashboard.vue`의 `shareInsight()`는 이미 `await`로 올바르게 처리 중이었음). 동일하게 `await`로 수정 |
| (2026.08.06) `chat_eval.py`(챗봇 행동 평가)가 적대적·엣지 시나리오를 `ask()`로만 돌려서, `ask_stream()`에서만 발생하는 회귀(이번에 실제로 발견된 예외 노출·done 누락 등)를 이 평가로는 영영 못 잡는 구조였음 | 스트림을 끝까지 소비해 `(mode, answer, evidence)`로 조립하는 `_run_stream()`을 추가하고, 적대적·엣지 검증을 `ask()`/`ask_stream()` 두 경로 모두 돌려 각각 통과율을 출력하도록 수정(한쪽이 더 낮으면 경고 출력). 실제로 `python -m chat.chat_eval` 실행해 두 경로 모두 8/8(100%)로 일치하는 것을 확인 |
| (2026.08.06, 교차검증에서 발견 — 위 Postgres 원자성 수정 자체가 만든 새 회귀) `RL_DB=postgres`로 돌리는 실제 배포(`/etc/systemd/system/reviewlens.service`)에서, 동시 요청 중 하나가 실패하면 그와 무관한 다른 요청의 쓰기(예: 피드백 저장)까지 조용히 함께 유실될 수 있음 | `_PgConn`을 `autocommit=False`로 바꾼 게 원인 — 이 커넥션은 `api/deps.py`에서 프로세스 전역으로 공유되고 여러 스레드가 동시에 쓰는데, 잠금은 문장 하나 단위인 반면 트랜잭션은 커밋 전까지 여러 문장(=여러 요청)에 걸쳐 열려 있어, 한 요청의 실패로 인한 롤백이 아직 커밋 전인 다른 요청의 쓰기까지 함께 날려버릴 수 있었음(게다가 `board()` 등 읽기 전용 요청은 커밋을 아예 안 해서 트랜잭션이 계속 열려있는 상태로 남기 쉬웠음). 공유 커넥션은 다시 `autocommit=True`(문장 하나 = 커밋 하나, 요청 간 절대 안 섞임)로 되돌리고, 진짜 원자성이 필요한 곳(`pipeline.py`의 리뷰별 delete+insert)만 명시적 `d.transaction()` 블록으로 분리(psycopg는 autocommit 커넥션에서도 `Connection.transaction()`으로 임시 트랜잭션을 열 수 있음 — psycopg 소스로 직접 확인). 목(mock) psycopg로 트랜잭션 커밋/롤백·재진입 잠금(RLock, 데드락 방지)까지 재검증(`tests/test_pgconn_transaction.py`) |
| (2026.08.06, 교차검증에서 발견) 비스트리밍(`/api/chat`)에서 정정 메모리 조회(`recall_corrections`)가 실패하면 그대로 예외가 터져 500 에러가 남 | `ask_stream()`은 이 호출을 try/except로 감쌌는데 `ask()`엔 그 보호가 빠져있었음(오늘 다른 수정에서 파생된 비대칭). 동일하게 try/except로 감싸도록 수정, 실패해도 direct 답변이 정상 반환되는 것을 확인(`tests/test_chat_cache.py`) |
| (2026.08.06, 교차검증에서 발견) FB(피드백) 모델에 `correction` 1000자 제한을 걸었는데 프론트 정정 입력창엔 `maxlength`가 없어서, 1000자 넘는 정정을 입력해 제출하면 422로 거부되고 프론트가 이를 조용히 삼켜(부가 기능이라 실패해도 대화는 계속돼야 한다는 기존 정책) 사용자는 아무 이유도 모른 채 피드백(투표 포함) 자체가 저장 안 됨 | `Chat.vue`의 정정 입력창에 `maxlength="1000"`을 추가해 백엔드 제한과 맞춤(질문 입력창의 `maxlength="500"`과 동일한 패턴) |
| (2026.08.06, 교차검증에서 발견) `tests/test_spa_fallback.py`의 SPA 라우트 테스트가 `frontend/dist`가 미리 빌드돼 있어야만 통과 — `npm run build`를 안 돌린 신규 체크아웃/CI에서는 이 테스트가 그냥 깨짐 | `app.py`는 `dist/index.html`이 없으면 의도적으로 503을 반환하는데, 테스트는 200만 기대하고 있었음. `dist`를 실제로 치워보고 503이 나는 것과, 그 상태에서도 이 테스트가 검증하려는 핵심(경로가 `/api/` 404 분기로 잘못 새지 않음)은 여전히 성립함을 확인한 뒤, 200/503 둘 다 허용하도록 수정 |
| (2026.08.07, 2차 교차검증에서 발견) `python pipeline.py`에서 상품 목록(item) 15건 시딩에 커밋 경계가 없어, 뒤이은 첫 리뷰의 `d.transaction()` 블록이 실패하면(예: CSV 첫 행의 평점 값이 숫자가 아님) 무관한 item 15건까지 함께 롤백됨 | SQLite는 커밋 없이 실행한 쓰기가 암묵적 트랜잭션에 계속 쌓이는데, item 시딩 루프엔 자체 커밋이 없어 뒤이은 리뷰 루프의 트랜잭션에 무임승차하고 있었음(오늘 세션 초반 Postgres에서 겪은 것과 정확히 같은 클래스의 문제가 SQLite에서 단일 프로세스 안에서도 재현됨). item 시딩 루프를 `with d.transaction():`으로 감싸 리뷰 루프와 트랜잭션 경계를 분리. 첫 리뷰의 평점을 일부러 숫자가 아닌 값으로 만들어, 수정 전엔 item 15건이 전부 사라지고 수정 후엔 그대로 남는 것을 확인(`tests/test_pipeline_lock.py`) |
| (2026.08.07, 2차 교차검증에서 발견) `ask()`가 이미 정상 생성·자기검증까지 끝낸 답변을, 그 뒤의 캐시 저장(`_cache_put`)이 실패하면(임베딩 모델 일시 오류 등) "답변을 생성하는 중 문제가 생겼어요"라는 오답으로 통째로 버림(원인도 로그에 "LLM 생성 실패"로 잘못 남음) | `_generate()`부터 `_cache_put()`까지 하나의 try 블록으로 묶여 있었음. `_cache_put()`을 별도 try/except로 분리(ask_stream()의 LLM 경로는 이미 이렇게 돼 있었음). 정상 생성된 답변에 캐시 저장만 실패하는 상황을 재현해, 수정 전엔 오답으로 대체되고 수정 후엔 정상 답변이 그대로 반환되는 것을 확인(`tests/test_chat_cache.py`) |
| (2026.08.07, 2차 교차검증에서 발견) 즉답(집계/추천/상품 등) 경로의 캐시 저장이 실패하면 `ask()`는 500 에러로, `ask_stream()`은 `done` 없이 스트림이 끊김 — `recall_corrections()`는 이번 세션에 양쪽 다 방어했는데 바로 옆의 `_cache_put()` 호출은 두 함수 모두 방치돼 있었음 | 두 함수의 즉답 경로 `_cache_put()` 호출 모두 try/except로 감싸 실패해도 이미 만든 즉답이 정상 반환·정상 `done`으로 이어지도록 수정. `_cache_put()`만 실패하는 상황을 재현해 확인(`tests/test_chat_cache.py`) |
| (2026.08.07, 2차 교차검증에서 발견) `tests/test_spa_fallback.py`가 `app` 모듈을 import하면 `app.py`의 예열(prewarm) 백그라운드 스레드가 실행되어, 실제(monkeypatch 안 된) 챗봇 캐시 상태를 건드림 — 마침 `test_chat_cache.py`가 쓰는 질문과 정규화 키가 완전히 같아, 타이밍에 따라 테스트 간 상태가 섞일 수 있는 잠재적 flaky 구조(이번엔 우연히 안 터짐) | `RL_SKIP_PREWARM` 환경변수를 `tests/conftest.py`(모든 테스트 파일보다 먼저 로드됨)에서 켜두고, `app.py`가 이 값이 있으면 예열 스레드를 시작하지 않도록 수정 |
| (2026.08.07) `/api/board`가 kpi·속성별·상품별 등 10여 개 SELECT를 트랜잭션 없이 순차 실행해서, `python pipeline.py`가 동시에 도는 중이면 응답 안 kpi 합계와 aspects/products 합계가 서로 다른 시점 데이터를 섞어 안 맞을 수 있음(그동안 "제대로 고치려면 실 Postgres 검증이 필요해서" 보류해뒀던 항목) | `d.transaction()`만으론 부족함을 먼저 확인 — Postgres 기본 격리수준(READ COMMITTED)은 트랜잭션으로 묶어도 문장마다 자기 시작 시점 기준 최신값을 봐서 다중 SELECT 일관성을 안 보장함. 이 블록에서만 **REPEATABLE READ**로 격상했다 되돌리는 `d.snapshot()`을 `_SqliteConn`(WAL 모드 명시적 BEGIN)·`_PgConn`(psycopg `isolation_level` 임시 전환, 트랜잭션 진행 중 변경 시도 시 psycopg 자체가 예외를 던지는 것까지 소스로 확인) 양쪽에 구현. `board()` 전체를 이 블록으로 감쌈. SQLite는 스냅샷 도중 다른 커넥션이 커밋해도 그 블록 안에서는 안 보이다가 블록이 끝나면 바로 보이는 것을 실제 파일로 재현·확인, Postgres는 psycopg를 목으로 대체해 REPEATABLE READ 격상→원복 시퀀스(예외 발생 시에도 원복됨 포함)를 검증(`tests/test_db_snapshot.py`) |
| (2026.08.08) `/api/board` 호출이 실패하면(서버 일시 오류 등) 대시보드가 "데이터를 불러오는 중…"에서 새로고침 전까지 영원히 안 넘어감 | `useBoard()`는 실패 시 `error` ref를 세팅해주는데 `Dashboard.vue`가 그걸 구독을 안 하고 있었고, 재시도 UI도 없었음. `error`를 구독해 실패 시 안내 문구 + "다시 시도" 버튼을 보여주도록 수정. Playwright 헤드리스 브라우저로 실제 dev 서버를 띄워 백엔드 응답 실패/성공 두 상태를 스크린샷까지 찍어 확인(재시도 버튼 클릭 시 재요청 나가는 것도 확인) |
| (2026.08.08) 존재하지 않는 프론트 경로(`/asdf` 등)로 직접 접속하면 헤더·네비 없는 완전 빈 화면이 뜸 | `main.js`의 vue-router에 와일드카드(404) 라우트가 없어, 서버(`app.py`)는 SPA 폴백으로 `index.html`을 정상 반환하는데 클라이언트 라우터가 매칭되는 라우트를 못 찾아 `<router-view />`가 아무것도 렌더링 안 하고 있었음. `NotFound.vue`를 추가하고 `{ path: '/:pathMatch(.*)*', component: NotFound }`로 등록. Playwright로 실제 렌더·홈 링크 동작까지 확인 |
| (2026.08.08) 죽은 코드 정리 | `pyflakes`로 백엔드 전체를 정적분석하고, 프론트는 모든 `import { A, B }` 심볼의 파일 내 등장 횟수를 스크립트로 대조해 전수 조사 — `tests/test_spa_fallback.py`의 안 쓰는 `import pytest` 1건 외에는 실제로 죽은 코드가 없음을 확인(ABSA/추천 대체 모듈들은 전부 README에 문서화된 정식 CLI 진입점, CSS 클래스들은 `lib/viz.js`를 통해 동적 바인딩되는 것까지 확인). 그 import 하나만 제거 |
