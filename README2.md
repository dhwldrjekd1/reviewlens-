...
| 18 | 단위테스트 + README 앞문 재구성 | pytest 19 / 헤더·결과·스크린샷 우선 (→§22) |
| 19 | UI 레퍼런스 매칭 + 폴리시 | 랜딩·챗봇·도넛·응대가이드 / 폰트↑·공백 제거 (→§22) |
| 20 | 챗봇 마케팅 어시스턴트 확장 | 12→17 의도(재구매·개선·강점·카피·답글) (→§22) |
| 21 | HF Spaces 무료 배포 준비 | Docker 멀티스테이지 + frontmatter (→§22) |
...
## 22. 후반 폴리시 · 확장 · 배포 준비 (마무리)

방향 확정 이후, *새 기능*보다 **완성도·발견성·제출 준비**에 집중.

**22a. 단위테스트(pytest 19):** 모델·Ollama 불필요한 순수 로직만 — DB 방언 변환·속성 추출·챗봇 의도 라우팅. CI에서도 빠르게 도는 신뢰도 시그널.

**22b. README 앞문 재구성:** 외부 AI 평가("개발자 문서 같다·너무 길다")를 *선별 수용* — **내용은 안 깎고 순서만**. 최상단에 포트폴리오 헤더(담당역할 100%·기간·스택·데모) + 한 줄 결과 + 스크린샷을 올리고, 무거운 ML 지표는 하위 유지. → HR 30초 첫인상↑ + 테크리드용 깊이 보존.

**22c. UI 레퍼런스 매칭 + 폴리시:** 제공된 디자인 시안에 맞춰 랜딩·챗봇·리뷰탭(감성 도넛·리뷰 응대 가이드)·마케팅탭을 화면별로 정렬(헤드리스 크롬 스크린샷 대조). 이어 전반 폰트 확대·박스 공백 제거·전략보드 게이지/버블맵 확대.

**22d. 챗봇 마케팅 어시스턴트 확장(12→17 의도):** "재구매 의향·개선 우선순위·강점/셀링포인트·광고 카피·리뷰 답글" 핸들러 추가 — 전부 ABSA 데이터에서 *정직하게* 도출. **단, 트렌드·세그먼트·캠페인 성과처럼 *데이터가 없는* 마케팅 기능은 가짜 답변이 되므로 의도적으로 제외**(정직함 = 핵심 강점 보존). 추천칩도 새 기능 노출용으로 교체.

**22e. 배포 준비(HF Spaces 무료):** Docker 멀티스테이지(Node 빌드 → Python 서빙) + README frontmatter(sdk: docker, app_port 7860). 저장소는 1.3MB만 push, 런타임은 컨테이너가 설치. Ollama 없이도 대시보드·챗봇 대부분 동작(사전계산), 자유서술형만 폴백.

> **마무리 원칙:** "더 만들기"가 아니라 *만든 걸 보이게(스크린샷·칩·헤더) + 정직하게(없는 데이터 가짜답변 금지) + 배포*. 남은 단 하나 = 라이브 배포.
